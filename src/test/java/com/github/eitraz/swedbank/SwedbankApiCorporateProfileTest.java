...
class SwedbankApiCorporateProfileTest {
    ...
    static void setupClientAndLogin() throws Exception {
        ...
        api = new MobileBankId(BankType.SWEDBANK_FORETAG)
                .authenticate(userId)
    ...
    void getAccountDetails() throws Exception {
        ...
        assertThat(details.getBalance().getAmount()).isNotNull();
    }
    ...
    void getTransactionsIterable() throws Exception {
        ...
        for (Transaction transaction : transactions) {
            ...
            // Transaction ID may be null
            if (transaction.getId() != null) {
                transactionId = transaction.getId();
            }
            // Use date, amount and details message when missing transaction ID
            else {
                transactionId = transaction.getDate() + "." + transaction.getDetails().getMessage() + "." + transaction.getAmount();
            }
